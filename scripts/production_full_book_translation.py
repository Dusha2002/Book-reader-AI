...
SOURCE = Path(os.getenv("BOOKAI_FULL_SOURCE") or "Devices_and_Desires.fb2")
OUTPUT = Path(os.getenv("BOOKAI_FULL_OUTPUT") or "Devices_and_Desires_RU_PRODUCTION_FULL.fb2")
...
ENTRYPOINT = Path("scripts/production_literary_translation.py")
...
def _run_chapter(name: str, timeout_seconds: int) -> dict[str, Any]:
    ...
    if status not in {"complete", "needs_review", "unknown"}:
        raise RuntimeError(f"unexpected chapter status={status!r}: {name}")
...
def main() -> int:
    ...
    try:
            ...
            _write_report(
                {
                    ...
                    "review_required_chapters": [
                        row["chapter"] for row in chapter_results if row.get("status") == "needs_review"
                    ],
                }
            )
        ...
        review = [row["chapter"] for row in chapter_results if row.get("status") == "needs_review"]
