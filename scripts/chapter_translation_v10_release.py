...
def main() -> None:
    ...
    print("[v10-release] " + json.dumps({
        "chapter": chapter_name,
        "segments": len(targets),
        "source_chars": sum(len(s.text) for s in targets),
        "whole_book_segments": len(all_targets),
        "selection": selection,
        "architecture": "clean-v10-release-hardening:complete-chapter+source-only-canon+technical-contracts+dialogue-gate+residual-release-tail",
    ...
    memory, bible_stats = HardenedBookBibleBuilder(giga, BIBLE).build(all_targets)
    ...
    provider = _provider()
    specialist = HardenedDeepSeekSemanticSpecialist(
        provider,
        qa,
    ...
    release_pre_issues = qa.scan(targets, translated, memory)
    release_local = HardenedLocalRewriter(giga, qa, max_segments=18)
    release_local_changed = release_local.repair(targets, translated, memory, release_pre_issues)
    usage_after_release_local = giga.usage.as_dict()
    dialogue_after_release_local = dialogue_guard.apply(targets, translated)
    speaker_release_local = DialogueSpeakerContinuityGuard()
    speaker_after_release_local = speaker_release_local.apply(targets, translated)

    release_mid_issues = qa.scan(targets, translated, memory)
    release_specialist = HardenedDeepSeekSemanticSpecialist(
        provider,
        qa,
        max_segments=max(4, int(os.getenv("BOOKAI_V10_RELEASE_DEEP_MAX") or "8")),
    )
    ...
    report = {
        "version": "v10-clean-8-release-hardening",
        "chapter": chapter_name,
        ...
        "architecture": {
            "chapter_selection": "numbered Chapter N boundary; internal FB2 section headings remain inside the selected chapter",
            "book_bible": "SOURCE-ONLY spelling canon with 2+ occurrence recovery plus technical anchors for fencing/mechanical terms",
            "primary": "GigaChat tagged batches with bounded recovery and deterministic Russian ordinal chapter headings",
            "dialogue": "source-aware Russian dialogue normalization plus hard publication typography gate",
            "qa": "quantity false-positive suppression + names + materials + half-inch + ethnonym + technical-denotation contracts",
            "release_tail": "post-integrity Giga local repair + bounded DeepSeek residual repair + second structural gate",
            "final_gate": "zero untranslated + zero structural integrity + zero HARD QA issues",
        ...
        "dialogue_guard": {
            **dict(dialogue_guard.stats),
            ...
            "after_release_local_changed_ids": dialogue_after_release_local,
        ...
        "speaker_guard": {
            ...
            "after_release_local": {**dict(speaker_release_local.stats), "changed_ids": speaker_after_release_local},
        ...
        "release_local_rewriter": {**release_local.stats, "changed_ids": release_local_changed},
    ...
    print("[v10-release-done] " + json.dumps(report, ensure_ascii=False), flush=True)
