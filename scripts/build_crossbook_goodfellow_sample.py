...
def _memory_rows(blocks: list[str]) -> list[str]:
    rows: list[str] = []
    for block in blocks:
        text = _norm(block)
        if len(text) < 45:
...
def main() -> None:
    ...
    memory_rows = _memory_rows(blocks)
    ...
    (out_dir / "sample.fb2").write_text(
        _fb2(selected, title="Deep Learning — short anti-overfit benchmark", document_id="bookai-crossbook-goodfellow-short"),
        "utf-8",
    )
    (out_dir / "memory-source.fb2").write_text(
        _fb2(memory_rows, title="Deep Learning — stable whole-book memory source", document_id="bookai-crossbook-goodfellow-memory"),
        "utf-8",
    )
